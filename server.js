...
http.createServer((req, res) => {
  ...
  if (!target.startsWith(path.join(__dirname, 'public')) || !fs.existsSync(target)) { res.writeHead(404); return res.end('Not found'); }
  const type = file.endsWith('.css') ? 'text/css' : file.endsWith('.js') ? 'application/javascript' : 'text/html'; res.writeHead(200, { 'Content-Type': type }); fs.createReadStream(target).pipe(res);
}).listen(PORT, () => console.log(`TakeOFF running on http://localhost:${PORT}`));
